...
#check install src
 if [ -z "${INSTALL_SRC}" ]; then
   ...
   if [ "${ARCH}" == "aarch64" ]; then
       INSTALL_SRC="https://bin.pkgforge.dev/aarch64_arm64_Linux" ; export INSTALL_SRC="${INSTALL_SRC}"
   elif [ "${ARCH}" == "x86_64" ]; then
       INSTALL_SRC="https://bin.pkgforge.dev/x86_64_Linux" ; export INSTALL_SRC="${INSTALL_SRC}"
   fi
...
#minisgn: https://github.com/jedisct1/minisign
 eval "${INSTALL_PRE}" "${INSTALL_SRC}/minisign" -o "${INSTALL_DIR}/minisign" && eval "${INSTALL_POST}" "${INSTALL_DIR}/minisign" "${INSTALL_STRATEGY}"
...
#xz:
 eval "${INSTALL_PRE}" "${INSTALL_SRC}/Baseutils/xz/xz" -o "${INSTALL_DIR}/xz" && eval "${INSTALL_POST}" "${INSTALL_DIR}/xz" "${INSTALL_STRATEGY}"
 eval "${INSTALL_PRE}" "${INSTALL_SRC}/Baseutils/xz/unxz" -o "${INSTALL_DIR}/unxz" && eval "${INSTALL_POST}" "${INSTALL_DIR}/unxz" "${INSTALL_STRATEGY}"
#Yq
